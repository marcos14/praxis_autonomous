...
- [x] tiles (gasto no mês, ativas, fases 7d, integradas no mês, franquia) via `metrics_dia`/view
- [x] gráfico de gastos por dia + tabela por projeto
- [x] lista "Precisa de você" (perguntas/aprovação/conflito) + atividade recente (SSE)
**Depende de:** 1h, 2a
...
- [x] branch publicada; card mostra branch, commits, preview de conflito com a main e link "abrir MR" (`url_plataforma`)
- [x] `GET /api/v1/demands/{id}/merge-preview`
**Depende de:** 2f, 2g
...
- [x] ação `integrar` → `merge --no-ff` local na main (com preview); ação `atualizar_branch` (traz main para a branch)
- [x] conflito → status `conflito` + lista de arquivos; resolver via agente ou manual
**Depende de:** 1g, 2g
...
- [x] limpeza de worktree/branch pós-integração (no MR, após merge detectado na main)
- [x] porte de `internal/notify` (de `notificacoes.go`/`notificar.go`): eventos do banco → Telegram/Discord/Slack/Google Chat
**Depende de:** 4c, 4d
...
- [x] porte de `auth.go` → `internal/api/auth.go`; `api_tokens` com papel (leitor/operador/admin)
- [x] `POST/DELETE /api/v1/tokens`; Bearer com papéis nos endpoints
- [x] `POST /projects/{id}/demands` (intake automatizado) conduz até "Aguardando respostas"
**Depende de:** 3b
...
- [x] seção Manual (conteúdo-base do protótipo) + `GET /api/v1/manual/*`
**Depende de:** 1h
...
- [x] interseção de `arquivos_provaveis` + `git diff --name-only` entre branches
- [x] badge no card/kanban com as demandas em sobreposição
**Depende de:** 1g, 4a
...
- [x] instalação como serviço Windows (e systemd) do `praxis.exe serve`
- [x] retenção de logs/eventos e backup periódico de `praxis.db`
**Depende de:** 1a, 1b
...
- [x] ler `autopilot.json`/`fases.csv` do Praxis atual e criar `projects`/config correspondentes (idempotente, uma vez)
**Depende de:** 1a, 1c
...
| 4b   | Home com métricas e "Precisa de você" | Concluída (gates verdes) | (pelo orquestrador) | 2026-07-17 | Métricas calculadas **direto das tabelas de origem** (runs/demands/phases) — `metrics_dia` não é pré-requisito (não é populado ainda). `db.ResumoHome(inicioMes, corte7d, corteGrafico)` → `{gasto_mes, demandas_ativas, fases_concluidas_7d, integradas_mes, aguardando_franquia, gastos_por_dia[], por_projeto[]}`; datas de corte via `substr(col,1,10)`; "ativas" = status não-terminal (exclui integrada/cancelada/falhou); tile de franquia é proxy (COUNT aguardando_franquia — espelho por conta é a pendência 1a.n1). `db.ListarDemandasPorStatus`. API: `GET /api/v1/metrics` (corta datas de `time.Now`, janela do gráfico 14d, devolve `hoje`), `GET /api/v1/pendencias` (aguardando_respostas/aguardando_aprovacao/conflito), `GET /api/v1/activity?limite=` (backlog de eventos). Frontend: view **Home** (`web/js/home.js`) — tiles, gráfico de barras CSS, tabela por projeto, "Precisa de você" (abre card), atividade recente com SSE global ao vivo (prepend). Home é a view default. |
| 4c   | Fechamento modo `merge_request` | Concluída (gates verdes) | (pelo orquestrador) | 2026-07-17 | `Servidor` ganha `git *gitops.Ops` (Opcoes.Git, default `gitops.Novo()`). `GET /api/v1/demands/{id}/merge-preview` → `{branch, base, modo_integracao, url_mr, commits_nao_publicados, commits[], limpo, conflitos[], aviso}`; preview via `gitops.PreviaMerge` (best-effort — erro vira `aviso`, não 500), commits via novo `gitops.CommitsAFrente(repo,base,branch)` (`git log base..branch`, `%h\x1f%s`), não-publicados via `CommitsNaoPublicados`. `montarURLMR`: GitHub `/compare/base...branch?expand=1`, senão GitLab `/-/merge_requests/new?...source_branch`. Demanda sem branch → 409 `sem_branch`. Ação `publicar_branch` wired em `POST /actions` (push manual tolerante, 502 em falha, eventos `branch_publicada`/`push_falhou`). Frontend: aba **Integração** no card (`web/js/demandas.js`, só com branch; default em conflito/concluída) — branch, preview de conflito, commits não publicados+Publicar, link Abrir MR, lista de commits. Ações `integrar`/`atualizar_branch` já surgem na aba (backend na 4d). |
| 4d   | Fechamento `merge_local`, conflito e atualizar branch | Concluída (gates verdes) | (pelo orquestrador) | 2026-07-17 | Ações no `POST /actions`: **`integrar`** (só `merge_local`) — `PreviaMerge(base,branch)`; limpo → `MergeNoFF` na main + status `integrada`; conflito → status `conflito` + arquivos no campo `erro` + 409 `{erro:{codigo:conflito, arquivos, demanda}}` sem tocar a main; modo `merge_request` recusa (409 `modo_invalido`). **`atualizar_branch`** (ambos os modos) — fetch se houver remote (usa `origin/<base>`), `PreviaMerge(branch, ref)`; limpo → novo `gitops.MergeNaBranch(worktree, ref)` (`git merge --no-edit`, aborta em conflito) e sai de `conflito`→`concluida`; conflito → status `conflito` + 409. Helpers `baseDoProjeto`/`marcarConflito`/`responderJSONConflito`. Frontend: aba Integração já traz os botões Integrar/Atualizar branch (4c) — agora funcionais; conflito aparece no banner + status. Limpeza pós-integração é 4e. |
| 4e   | Limpeza pós-integração e notificações | Concluída (gates verdes) | (pelo orquestrador) | 2026-07-17 | **Limpeza:** `gitops.RemoverBranch` (guarda `praxis/*`, `git branch -D`) + `gitops.BranchIntegrada` (0 commits à frente da base = mesclada). `s.limparPosIntegracao` remove worktree ANTES da branch (best-effort, evento `limpeza_pos_integracao`); chamada no `integrar` (merge_local, zera worktree_path) e em `s.reconciliarMR` (merge_request) — reconciliação preguiçosa no GET merge-preview: fetch + `BranchIntegrada(origin/base)` → marca `integrada` + limpa; resposta ganha `status`/`ja_integrada`. **Notificações:** pacote `internal/notify` portado de `notificacoes.go`/`notificar.go`: `Config{canais,eventos,cabecalho}`, `Notificador.Enviar` (Telegram/Discord/Slack/Google Chat/webhook genérico via `postJSON`), `Despachante` que tail-a `EventosApos` e envia os tipos ligados. Config lida da config global (chave `notificacoes`); despachante **wired no `serve`** (`iniciarNotificacoes`, cursor inicial = último evento, hook `AoIniciar` p/ teste determinístico). Testes com mock HTTP. **Pendência:** editor de notificações na tela Configurações (config setável via `PUT /api/v1/config`). |
| 5a   | API de intake para sistema de chamados (tokens/papéis) | Concluída (gates verdes) | (pelo orquestrador) | 2026-07-17 | Migração **v5** `api_tokens(id,nome,token_hash UNIQUE,papel CHECK,criado_em,revogado_em)` — só o **hash SHA-256** é guardado; valor em claro mostrado 1x. `db/tokens.go`: `PapelLeitor/Operador/Admin`, `GerarTokenAPI`(rand+base64url), `HashToken`, `CriarToken`/`ListarTokens`(sem segredo)/`RevogarToken`/`AutenticarToken`/`ContarTokensAtivos`. `api/auth.go`: `tokenDaRequisicao` (Bearer + X-Praxis-Token, portado), middleware `comAuth` no chain (recover→log→auth). **Modelo:** SEM token = admin local (bind loopback é confiável; tokens são aditivos p/ chamadores remotos); token válido = seu papel (`leitor` barrado de escrita → 403); token inválido/revogado → 401. `papelRequeridoPara`: `/tokens*`→admin, GET→leitor, escrita→operador. `api/tokens.go`: `POST/GET /tokens`, `DELETE /tokens/{id}`. Intake automatizado já pronto (POST demands por chat → analista → aguardando_respostas). UI: painel **Tokens de API** na tela Configurações. |
| 5b   | Manual embutido | Concluída (gates verdes) | (pelo orquestrador) | 2026-07-17 | Conteúdo do manual portado do protótipo em 8 arquivos markdown (`internal/api/manual/NN-slug.md`), embutidos via `//go:embed`. `api/manual.go`: parse do título (`# ...`) e slug (sem prefixo NN-); `GET /api/v1/manual` (lista slug+título) e `GET /api/v1/manual/{slug}` (markdown), 404 em slug desconhecido. Frontend: view **Manual** (`web/js/manual.js`) com índice lateral (âncoras) + render markdown mínimo (parágrafos, listas, `**negrito**`, `` `código` ``) via nós DOM (sem innerHTML). Nav/rota adicionadas. |
| 5c   | Badge de sobreposição entre demandas | Concluída (gates verdes) | (pelo orquestrador) | 2026-07-17 | Conjunto de arquivos por demanda = `arquivos_provaveis` (do `meta` da fala do analista, via `db.ArquivosProvaveisDemanda`) ∪ `gitops.DiffNames(repo, base, branch)` (`git diff --name-only base...branch`). `api/overlap.go`: `mapaSobreposicoes` (par a par, **só entre demandas do mesmo projeto**, cache de projeto) e `sobreposicoesDe`; considera status ativos. `GET /api/v1/overlaps` (mapa demandID→[{demand_id,titulo,arquivos}] para os badges) e `GET /api/v1/demands/{id}/overlap` (detalhe). Frontend: badge `⚠ sobrepõe N` no card do kanban (title com arquivos) + banner de sobreposição no topo do card modal. |
| 5d   | Serviço Windows, retenção e backup | Concluída (gates verdes) | (pelo orquestrador) | 2026-07-17 | **Backup/retenção (automatizável, testado):** `db.BackupPara` (`VACUUM INTO` — backup online seguro em WAL); `db.RemoverEventosAntesDe`. Pacote `internal/manutencao`: `Manutencao.Ciclo` (backup carimbado → `RotacionarBackups` mantém N=7 → retenção de eventos + `LimparLogs` por idade, default 30d) e `Rodar` (loop 24h). **Wired no `serve`** (`iniciarManutencao`, backups em PRAXIS_HOME/backups). **Serviço (requer admin p/ registrar):** subcomando `praxis service` gera a **unit systemd** (Linux) ou o comando **`sc.exe create`** (Windows) — só imprime (registro efetivo é manual/admin, conforme a observação da fase). Geradores `unitSystemd`/`comandoServicoWindows` testados. |
| 5e   | Importador opcional do Praxis atual | Concluída (gates verdes) | (pelo orquestrador) | 2026-07-17 | Pacote `internal/importador`: `Importar(store, pasta)` lê `automacao/autopilot.json` (`configClassica`) + `fases.csv` (separador `;`, deps por `+`). Cria o projeto (nome de `projeto` ou basename, add_dirs, modo merge_request) + config de projeto mapeada (`budget_demanda_usd`←max_budget_usd, `max_correcoes`/`max_ciclos_revisao`/`max_fases_novas`, `motor_preferido`←motor/ordem[0], `gates`←comandos achatados). Fases **pendentes** (concluídas ignoradas) viram 1 demanda **pausada** com o `plano_md` (lido de `cfg.Plano`). **Idempotente:** projeto já existente na mesma pasta → no-op (`Criado=false`), sem duplicar projeto/demanda. Subcomando `praxis import <pasta>...`. Opcional — ativação é decisão de negócio (não roda no `serve`). |
